...
public class NavUtils {
    private static double EARTH_RADIS_IN_METERS = DistanceUtils.EARTH_MEAN_RADIUS_KM * 1000;
    ...
    public static double getBearing(Point pt1, Point pt2) {
        ...
        double longDiff = Math.toRadians(latitude2 - latitude1);
     ...
     */
     public static Point getPosition(Point pt1, double d, double brg) {
         ...
         double lat2 = Math.asin(Math.sin(lat1)) * Math.cos(d / EARTH_RADIS_IN_METERS) + Math.cos(lat1) *
                       Math.sin((d / EARTH_RADIS_IN_METERS) * Math.cos(brgAsRadians));
         double x = Math.sin(brgAsRadians) * Math.sin(d / EARTH_RADIS_IN_METERS) * Math.cos(lat1);
         double y = Math.cos(d / EARTH_RADIS_IN_METERS) - Math.sin(lat1) * Math.sin(lat2);
